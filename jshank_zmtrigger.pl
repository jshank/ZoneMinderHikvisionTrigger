...
sub handleMessage
{
	...
	if ( !$monitor )
	{
		Warning( "ARC:Can't find monitor '$id' for message '$message'\n" );
		# ARC - added check to reload monitors one time if monitor not found
		Warning ("ARC: Let me reload Monitors to make sure....\n");
		foreach my $monitor ( values(%monitors) )
        	{
            		# Free up any used memory handle
            		zmMemInvalidate( $monitor );
		...
		if (!$monitor)
		{
			Warning ("ARC:Tried reloading, still no luck! Maybe Monitor:$id is not in nodect ");
			return;
		}
		else
		{
			Warning ("ARC:Found that monitor after reloading!");
		}
		# ARC - end of changes
	}
